...
namespace BasketballStats.WebApi.Business
{
    public static class BusinessUtil
    {
        ...
        public static void CheckBranchValue(bool isRequired, List<string> additionalInfoList, List<object> valueList)
        {
            var count = valueList.Count(value => value != null);

            var additionalInfo = additionalInfoList.Aggregate(string.Empty, (current, value) => current + ("," + value));
